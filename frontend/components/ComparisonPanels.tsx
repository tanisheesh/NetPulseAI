    ...
    <div className={`grid grid-cols-1 ${hasRL ? 'lg:grid-cols-3' : 'lg:grid-cols-2'} gap-6 relative`}>
      ...
      {!hasRL && (
        <div className="hidden lg:block absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
          <div className="w-16 h-16 bg-gradient-to-br from-[#06b6d4] to-[#14b8a6] rounded-full flex items-center justify-center border-4 border-black shadow-lg">
      ...
      {/* Baseline Panel */}
      <div className="bg-[#0a0a0a] backdrop-blur-sm rounded-xl border-t-4 border-t-blue-500 border-x border-b border-[#1a1a1a] p-6 relative overflow-hidden">
        {/* Blue glow */}
      ...
      {/* AI Panel */}
      <div className="bg-[#0a0a0a] backdrop-blur-sm rounded-xl border-t-4 border-t-cyan-500 border-x border-b border-[#1a1a1a] p-6 relative overflow-hidden">
        {/* Cyan glow */}
        <div className="absolute top-0 left-0 right-0 h-32 bg-cyan-500/10 blur-2xl"></div>
        ...
        <div className="relative">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-right">
              <div className="text-xs text-[#64748b] mb-1">QoS Score</div>
              <div className="text-3xl font-bold text-cyan-400 drop-shadow-[0_0_10px_rgba(6,182,212,0.5)]">
                {aiQoS.toFixed(2)}
              </div>
            </div>
          </div>
      ...
      {/* RL Panel */}
      {hasRL && (
        <div className="bg-[#0a0a0a] backdrop-blur-sm rounded-xl border-t-4 border-t-teal-500 border-x border-b border-[#1a1a1a] p-6 relative overflow-hidden">
          {/* Teal glow */}
          <div className="absolute top-0 left-0 right-0 h-32 bg-teal-500/10 blur-2xl"></div>
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {tick < 100 && (
                  <span className="px-2 py-1 bg-teal-500/20 text-teal-400 text-xs rounded-full font-semibold animate-pulse">
                    Learning...
                  </span>
                )}
                {tick >= 500 && (
                  <span className="px-2 py-1 bg-teal-500/20 text-teal-400 text-xs rounded-full font-semibold">
                    Converged
                  </span>
                )}
              </div>
              <div className="text-right">
                <div className="text-xs text-[#64748b] mb-1">QoS Score</div>
                <div className="text-3xl font-bold text-teal-400 drop-shadow-[0_0_10px_rgba(20,184,166,0.5)]">
                  {rlQoS!.toFixed(2)}
                </div>
              </div>
            </div>
